...
- `services/objects.py` — thin `boto3` S3-client wrapper (`put_object`/`get_object`/`ensure_bucket`/`delete_prefix`) pointed at whatever `STORAGE_ENDPOINT` is configured — MinIO locally, Cloudflare R2 in prod. `delete_prefix()` bulk-deletes everything under a user's `uploads/{user_id}/` folder; `routers/account.py::delete_account` calls it after the DB delete so uploaded prescription photos don't outlive the account — Postgres FK cascades clean up the rows (`prescriptions`, `children`, `processing_jobs` all have `ON DELETE CASCADE` to `users`), but nothing else ever touched the underlying files in object storage.
